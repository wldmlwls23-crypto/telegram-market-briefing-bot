...
def send_telegram_photo(image: bytes, caption: str) -> None:
    ...
    response = requests.post(
        ...
        files={"photo": ("btc-1d.png", image, "image/png")},
        timeout=30,
    )
    response.raise_for_status()
...
def emergency_topic_key(item: dict[str, str]) -> str:
    text = normalize_text(f"{item.get('title', '')} {item.get('summary', '')}")
    today = datetime.now(KST).strftime("%Y-%m-%d")
    if any(term in text for term in ("oil", "wti", "crude")) and any(
        term in text for term in ("iran", "israel", "war", "attack", "middle east", "geopolitical")
    ):
        return f"emergency-topic:oil-geopolitical:{today}"
    if any(term in text for term in ("ethereum", "eth")) and any(
        term in text for term in ("plunge", "drop", "crash", "급락", "하락")
    ):
        return f"emergency-topic:eth-sharp-move:{today}"
    if any(term in text for term in ("bitcoin", "btc")) and any(
        term in text for term in ("plunge", "drop", "crash", "surge", "spike", "급락", "급등")
    ):
        return f"emergency-topic:btc-sharp-move:{today}"
    if any(term in text for term in ("usdt", "usdc", "stablecoin", "depeg")):
        return f"emergency-topic:stablecoin-risk:{today}"
    if any(term in text for term in ("binance", "coinbase", "exchange")) and any(
        term in text for term in ("hack", "withdrawal", "suspended", "regulator", "sec")
    ):
        return f"emergency-topic:exchange-structure:{today}"
    if any(term in text for term in ("fomc", "powell", "cpi", "pce", "jobs report", "nonfarm")):
        return f"emergency-topic:us-macro:{today}:{stable_key(text[:160])}"
    return f"emergency-topic:{stable_key(text[:220])}"
...
def fetch_btc_intraday_prices() -> list[tuple[datetime, float]]:
    response = requests.get(
    ...
    response.raise_for_status()
    points = []
    for timestamp_ms, price in response.json().get("prices", []):
        points.append((datetime.fromtimestamp(timestamp_ms / 1000, UTC).astimezone(KST), float(price)))
    if len(points) < 2:
        raise RuntimeError("Not enough BTC intraday data to draw a chart.")
    return points


def create_btc_chart_image(crypto_prices: dict[str, Any], generated_at: str) -> bytes:
    points = fetch_btc_intraday_prices()
    times = [point[0] for point in points]
    prices = [point[1] for point in points]
    current = float(crypto_prices.get("bitcoin", {}).get("usd") or prices[-1])
    change_pct = float(crypto_prices.get("bitcoin", {}).get("usd_24h_change") or 0)
    previous = current / (1 + change_pct / 100) if change_pct != -100 else prices[0]
    change_usd = current - previous
    color = "#22c55e" if change_pct >= 0 else "#ff4d45"

    fig, ax = plt.subplots(figsize=(10, 5.8), dpi=150)
    fig.patch.set_facecolor("#050505")
    ax.set_facecolor("#050505")
    fig.subplots_adjust(left=0.10, right=0.96, bottom=0.16, top=0.68)

    fig.text(0.06, 0.94, f"{generated_at[:10]} Morning Market Report", color="white", fontsize=16, weight="bold")
    fig.text(0.06, 0.875, "Bitcoin (BTC)  |  1D", color="#b8c0cc", fontsize=10)
    fig.text(0.06, 0.79, f"${current:,.2f}", color="white", fontsize=25, weight="bold")
    fig.text(
        0.06,
        0.725,
        f"{change_usd:+,.2f} USD  ({change_pct:+.2f}%)",
        color=color,
        fontsize=12,
        weight="bold",
    )
    ...
    ax.set_title("BTC price, last 24 hours (KST)", color="#d8dde5", fontsize=10, loc="left", pad=12)
    ...
    for name, symbol in INDEX_SYMBOLS.items():
        try:
            ...
            if close is not None and open_value not in {None, 0}:
                change_from_open = f"{((close - open_value) / open_value) * 100:.2f}%"
            results.append(
                {
                    "name": name,
...
def clean_calendar_value(value: Any) -> str:
...
def build_event_value_summary(event: dict[str, Any]) -> str:
    actual = clean_calendar_value(event.get("actual"))
    forecast = clean_calendar_value(event.get("forecast"))
    previous = clean_calendar_value(event.get("previous"))
...
def is_qualitative_event(title: str) -> bool:
    text = normalize_text(title)
    return any(term in text for term in ("speaks", "minutes", "statement", "rate decision", "press conference", "fomc"))
...
def fetch_economic_calendar(days_ahead: int = 1) -> list[dict[str, str]]:
    ...
    for event in events:
        ...
        value_summary = build_event_value_summary(event)
        title = str(event.get("title", ""))
        if not value_summary and not is_qualitative_event(title):
            continue
        ...
        filtered.append(
            {
                ...
                "title": title,
                "title_ko": korean_event_title(title),
                ...
                "value_summary": value_summary or "수치 발표 아님",
...
- "확인 필요"라는 표현 금지
- 값이 없는 자산은 생략
- 숫자가 있으면 기준값 대비 방향을 함께 작성. 예: "시가 100 → 현재 97, -3.0%"
- 지표 숫자는 단독으로 쓰지 말고 "무슨 지표 / 발표 시간 / 예상·이전치 대비 / 시장 의미"를 붙여 작성
- 경제일정은 value_summary를 반드시 포함. value_summary가 "수치 발표 아님"이면 수치 이벤트처럼 설명하지 말 것
- generated_at_kst보다 과거인 일정은 "주목", "예정", "발표 예정"으로 쓰지 말 것
- 이미 지난 일정은 실제 발표값(actual/result)과 시장 반응이 원자료에 있을 때만 "결과"로 쓰고, 없으면 제외
- 뉴스 나열 금지, 핵심 변수만 정리
...
인플레 부담, 고용 약화 신호, 유동성 둔화 신호, 관찰 우선순위
""".strip()
...
현재 시각보다 지난 경제일정을 앞으로 볼 이벤트처럼 쓰지 마세요.
예: 생성 시각이 06:50 KST인데 03:00 KST FOMC 의사록을 "주목할 것"이라고 쓰면 안 됩니다.
지난 이벤트는 실제 결과값과 시장 반응이 원자료에 있을 때만 결과로 짧게 쓰세요.

## 2. [Economic Calendar]
현재 시각 이후 오늘과 향후 3거래일의 ★★★★ 이상 일정만 최대 5개 정리하세요.
현재 시각보다 지난 일정은 절대 쓰지 마세요.
영어 지표명을 그대로 쓰지 말고 title_ko와 country_ko를 우선 사용하세요.
각 일정은 반드시 value_summary를 포함하세요.
예:
- 21:30 KST / 미국 / 근원 PCE 물가 / ★★★★★
  값: 예상 0.2% / 이전 0.1%
  의미: 높으면 인플레 부담·달러/금리 상승 압력, 낮으면 달러 약세 압력.
...
def clean_for_telegram(text: str) -> str:
    ...
    result = "\n".join(cleaned_lines)
    result = result.replace("확인 필요", "").replace("확인필요", "")
    result = re.sub(r"\n{3,}", "\n\n", result)
    return result.strip()
...
시장 반응:
- 원자료 또는 market_snapshot에 숫자가 있는 항목만 작성
- 숫자가 있으면 기준값 대비 방향을 작성. 예: "BTC: 24시간 60,100달러 → 58,900달러, -2.0%"
- 숫자가 없으면 해당 항목을 아예 쓰지 말 것
...
def create_pre_event_reminder(event: dict[str, str]) -> str:
    value_summary = event.get("value_summary") or "수치 발표 아님"
    return f"""[중요 이벤트 사전 알림 | ★★★★★]

{event['time_kst']} KST {event.get('country_ko', event['country'])} {event.get('title_ko', event['title'])} 발표 예정.
...
def send_report(report_type: str) -> None:
    try:
        ...
        if report_type == "morning":
            try:
                chart = create_btc_chart_image(
                    market_data["crypto_prices"],
                    market_data["generated_at_kst"],
                )
                send_telegram_photo(chart, f"{title}\nBTC 1D 가격 차트")
            except Exception:
                logging.exception("BTC morning chart failed; sending text report only.")
    ...
    try:
        ...
        for item in news:
            if item.get("importance") != "★★★★★":
                continue
            key = emergency_topic_key(item)
            if already_sent(key):
                continue
            alert = create_emergency_alert({**item, "market_snapshot": market_snapshot})
